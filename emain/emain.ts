...
console.log(
    sprintf(
        "snorkeling-app starting, data_dir=%s, config_dir=%s electronpath=%s gopath=%s arch=%s/%s electron=%s",
        waveDataDir,
        waveConfigDir,
        getElectronAppBasePath(),
        getElectronAppUnpackedBasePath(),
...
if (isDev) {
    console.log("snorkeling-app WAVETERM_DEV set");
}
...
electronApp.on("before-quit", (e) => {
    ...
    if (
        ...
        const choice = electron.dialog.showMessageBoxSync(null, {
            ...
            buttons: ["Cancel", "Quit"],
            title: "Confirm Quit",
            message: "Are you sure you want to quit Snorkeling?",
            defaultId: 0,
            cancelId: 0,
        });
        if (choice === 0) {
...
async function appMain() {
    ...
    if (!instanceLock) {
        console.log("snorkeling-app could not get single-instance-lock, shutting down");
        setUserConfirmedQuit(true);
        electronApp.quit();
        return;
    }
