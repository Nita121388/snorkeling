...
Snorkeling's AI system supports custom AI modes that allow you to use local models, custom API endpoints, and alternative AI providers. The configuration files and keys still use the inherited Wave AI naming, which keeps existing `waveai.json` and `waveai:*` settings compatible.

## Configuration Overview

AI modes are configured in `~/.config/snorkeling/waveai.json` by default. The config key names still use `waveai:*` for upstream compatibility.
...
The inherited Wave AI configuration supports provider-based settings that automatically apply sensible defaults for common providers. By specifying the `ai:provider` field, you can significantly simplify your configuration as the system will automatically set up endpoints, API types, and secret names.
...
Snorkeling supports the following AI API types:
...
You can configure global Snorkeling AI behavior in settings, separate from the mode configurations in `waveai.json`.

### Setting a Default AI Mode

After configuring a local model or custom mode, you can make it the default by setting `waveai:defaultmode` in Snorkeling settings.
...
This will make the specified mode the default selection when opening Snorkeling AI features.

:::note
Snorkeling currently inherits the upstream telemetry gate for built-in cloud AI modes. However, if you configure your own custom model (local or BYOK) and set `waveai:defaultmode` to that custom mode's key, you will not receive telemetry requirement messages. This allows you to use AI features privately with your own models. <VersionBadge version="v0.13.1"/>
:::

### Hiding Inherited Cloud Modes

If you prefer to use only your local or custom models and want to hide the inherited cloud AI modes from the mode dropdown, set `waveai:showcloudmodes` to `false`:
...
This will hide the inherited built-in cloud AI modes, showing only your custom configured modes.
...
The `ai:apitokensecretname` should be the name of an environment variable that contains your API key. Set this environment variable before running Snorkeling.
...
Instead of storing API keys directly in the configuration, you should use Snorkeling's secret store to keep your credentials secure. Secrets are stored encrypted using your system's native keychain.
...
The `openai` provider automatically looks for the `OPENAI_KEY` secret. See the [Secrets documentation](./secrets.mdx) for more information on managing secrets securely in Snorkeling.
...
If Snorkeling can't connect to your model server:
...
| `waveai:cloud` | No | Internal - inherited cloud AI configuration only |
| `waveai:premium` | No | Internal - inherited cloud AI configuration only |
...
:::warning
If you don't include `"tools"` in the `ai:capabilities` array, the AI model will not be able to interact with Snorkeling terminal widgets, read/write files, or execute commands. Most AI modes should include `"tools"` for the best Snorkeling experience.
:::
