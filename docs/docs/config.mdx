...
Snorkeling's configuration files are located at `~/.config/snorkeling/` by default.

The main configuration file is `settings.json` (`~/.config/snorkeling/settings.json`). In development builds this directory may use a `snorkeling-dev` suffix.
...
The easiest way to edit your config files is to use the `wsh editconfig` command, which opens your Snorkeling config file in the built-in preview editor.
...
| ------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| app:globalhotkey                     | string   | A systemwide keybinding to open your most recent Snorkeling window. This is a set of key names separated by `:`. For more info, see [Customizable Systemwide Global Hotkey](#customizable-systemwide-global-hotkey)                                                 |
| app:dismissarchitecturewarning       | bool     | Disable warnings on app start when you are using a non-native architecture for Snorkeling. For more info, see [Why does Snorkeling warn me about ARM64 translation when it launches?](./faq#why-does-snorkeling-warn-me-about-arm64-translation-when-it-launches).              |
...
| app:confirmquit <VersionBadge version="v0.14" />                     | bool     | Set to false to disable the quit confirmation dialog when closing Snorkeling (defaults to true, requires app restart)                                                                                                                                      |
...
| ai:proxyurl                          | string   | HTTP proxy URL for AI API requests (does not apply to inherited Wave Cloud AI modes)                                                                                                          |
| agent:defaultprofile                 | string   | default Agent profile to use when launching an Agent block                                                                                                                                                                                                    |
| agent:profiles                       | object   | map of Agent profile definitions, such as command, args, model args, and environment variables                                                                                                                                                                |
...
| block:inlinetabactivemaxwidthpct      | float    | max width (as a percentage of the block width) for the currently-opened inline-tab label. default 45. content longer than this scrolls inside the tab.                                                                                                          |
...
For reference, this is the current default configuration:
...
The current source default for `web:defaulturl` still points to the upstream Wave repository. You can set it to `https://github.com/Nita121388/snorkeling` if you want web widgets to open the Snorkeling repository by default.

:::

:::warning

If an old combined app-home directory exists, Snorkeling can still read compatible data from `~/.snorkeling/config/settings.json`. This includes other configuration files such as `termthemes.json`, `presets.json`, and `widgets.json`.

:::

## Environment Variable Resolution

To avoid putting secrets directly in config files, Snorkeling supports environment variable resolution using `$ENV:VARIABLE_NAME` or `$ENV:VARIABLE_NAME:fallback` syntax. This works for any string value in any config file (settings.json, presets.json, ai.json, etc.).
...
User-defined terminal themes are located in `~/.config/snorkeling/termthemes.json`.
...
Here is an example of defining a full terminal theme. Snorkeling currently inherits the upstream default terminal themes from the source tree.
...
Snorkeling allows setting a custom global hotkey to open your most recent window from anywhere in your computer. This has the name `"app:globalhotkey"` in the `settings.json` file and takes the form of a series of key names separated by the `:` character.

### Examples

As a practical example, suppose you want a value of `F5` as your global hotkey. Then you can simply set the value of `"app:globalhotkey"` to `"F5"` and reboot Snorkeling to make that your global hotkey.
