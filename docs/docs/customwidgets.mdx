...
| "action"        | (optional) Runs a built-in frontend action instead of creating a block. Currently supports `"commontext:search"` to open Common Text search.                                                                                                                                               |
| "blockdef"      | Defines the block created by the widget. Note that all further definition takes place inside a meta object inside this one. Required for widgets that do not use `"action"`.                                                                                                               |
...
| "cmd:cwd"              | (optional) A string representing the current working directory to run the command or shell on the target host. Local paths expand `~` on the local machine; remote paths keep `~` for the remote shell to expand. Defaults to the home directory.                               |
...
Wave ships with default widgets in the widgets bar (terminal, agent, files, sessions, Common Text, web, sysinfo, and processes). You can modify or remove these by overriding their config in widgets.json. The names of the default widgets, in order, are:
