...
Snorkeling allows users to connect to various machines and unify them together in a way that preserves the unique behavior of each. At the moment, this extends to SSH remote connections and local WSL connections.
...
`wsh` is a small program that helps manage Snorkeling regardless of which machine you are currently connected to. It is always included on your host machine, but you also have the option to install it when connecting to SSH and WSL Connections. If it is installed on the connection, it is installed at `~/.snorkeling/bin/wsh`. Then, when Snorkeling connects to your connection (and only when Snorkeling connects to your connection), the following happens:

- `~/.snorkeling/bin` is added to your `PATH` for that individual session. This allows the user to use the `wsh` command without providing the complete path.
...
If this fails for some reason, Snorkeling will attempt to run without `wsh`. You will see this indicated by a small **<code><i className="fa-link-slash fa-solid fa-sharp"/></code>** icon in the block header. For more info on what `wsh` is capable of, see [wsh command](/wsh). The current CLI name is still `wsh` for compatibility.
...
| -------------------- | ----------------------------------------------------------------------------- |
| TERM_PROGRAM         | Currently set to `waveterm` for upstream compatibility.                       |
| WAVETERM             | This is set in Snorkeling-managed sessions for upstream compatibility.         |
...
| WAVETERM_VERSION     | The current Snorkeling app version.                                           |
| WAVETERM_WORKSPACEID | The id of the workspace containing your current terminal widget.              |
...
In addition to the regular ssh config file, Snorkeling also has its own config file to manage separate variables. These include
| Keyword | Description |
|---------|-------------|
...
| conn:wshpath | A string indicating the path to the `wsh` executable on the connection. It defaults to `"~/.snorkeling/bin/wsh"`.|
...
| conn:ignoresshconfig | This boolean allows Snorkeling to ignore the `~/.ssh/config` file for resolving keywords for this connection. The regular defaults will be used, but all changes to those must be specified in the `connections.json` file instead. This defaults to false.|
...
Snorkeling resolves the identity agent path in this order:
...
If this is not sufficient, it is also possible to view the full log file. To do this, you can run the command `wsh wavepath log` to get the location of a log file. The subcommand name is still `wavepath` for compatibility.
...
If the shell is different than those, it is possible the `wsh` command will not work by default. The easiest way to fix this at the moment is to switch the shell type. This can be done by setting the `conn:shellpath` value with a path to one of the above shells in the `connections.json` file for the connection you are trying to use. Alternatively, you can use the `chsh` command to change the shell in that connection, but this will also take effect outside of Snorkeling. Once this is done, restart Snorkeling for the changes to take effect.

#### AllowTcpForwarding in sshd

Some systems have sshd configured to disable TCP forwarding by default. This can be found on the connection in the `/etc/ssh/sshd_config` file. In that file, search for the line containing `AllowTcpForwarding`. If this is set to `no`, it is likely the reason `wsh` will not work on your connection. In order to get `wsh` working, set the value for `AllowTcpForwarding` to either `yes` or `local` (they both provide different levels of permission but both work in this case). Then, restart the `sshd` service with whichever method your remote machine provides. Once that is done, restart Snorkeling so it can reconnect with this change.
