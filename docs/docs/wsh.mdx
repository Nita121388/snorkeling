...
The `wsh` command provides Snorkeling's core command line interface. The command name is inherited from upstream Wave for compatibility, and it lets you interact with both terminal and graphical elements from the command line. This guide covers the basics of using `wsh` and its key features.

See the [wsh reference](/wsh-reference) for a list of all wsh commands and their arguments.

## Overview

At its core, `wsh` enables seamless interaction between your terminal commands and Snorkeling's graphical blocks. It allows you to:
...
`wsh` provides direct interaction with Snorkeling's graphical blocks through the command line. For example:
...
Every visual element in Snorkeling is a block, and `wsh` gives you complete control over them (hold Ctrl+Shift to see block numbers):
...
The `wsh ai` command appends content to the Snorkeling AI sidebar. By default, files are attached without auto-submitting, allowing you to review and add more context before sending.
