...
| P06 | 🟢 | `design/prototypes/_to-keep/commontext-search-virtual.html` | 深度巡检 Common Text 模块：原型主张虚拟滚动，真实代码只采纳 limit 500（未真虚拟化），原型作为**待改进提案**保留（设计资产）——属健康状态，无需修 | **推荐：✅ 无需处理（设计资产）**。仅记录：若未来做真虚拟化，此原型是现成参考 | 深度巡检：`commontext-compose-modal.tsx:29 LIST_LIMIT=500`，无虚拟滚动实现 | 2026-08-04 |

> 此处为实时截图示：
> `node design/prototypes/audit-sync.mjs` 自动生成新的 🔴/🟡 条目后，agent 把它们追加到这里，
