...
| 文件 | 作用 |
|---|---|
| `sync-governance.md` | 系统设计（v1.3：功能模块对齐 / 深度+增量巡检 / 回滚 / 时间管理 / 进化Log） |
| `modules.md` | **功能模块基线**（三处：代码⇄原型⇄方案，核心/次要/待清理） |
| `TODOS.md` | 登记表（待审批 / 已关闭 / 已拒绝 / 💬待回复 / 挂起 / 已自动修正） |
| `approval-ui.html` | 审批界面（双击打开，逐条 / 批注 / 批量绿灯） |
| `audit-sync.mjs` | 声明层辅助检查（地址失效/版本/日期，`--json` 供审批） |
...
```
对 agent 说：「巡检」或「检查原型」
→ 按 modules.md 定位受影响模块（深度：核心模块优先）
→ 三处对齐检查（代码/原型/方案）
