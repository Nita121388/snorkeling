...
![将一个 block 拖到另一个 block 上方，占位提示显示将投放到目标 block 父节点之前（外侧投放）](./img/drag-edge.png)
...
![将一个 block 拖到另一个 block 上方，占位提示显示将交换两者位置](./img/drag-swap.png)
![标注示例，展示一个 block 内的投放目标](./img/block-drag-example.jpg)
...
![光标悬停在 block 边距上时出现的线条，指示拖动该边距将调整哪些 block
的大小](./img/node-resize.png)
...
除了上游 Wave 的上述布局能力，Snorkeling fork 还增加了几项面向工作流的布局能力。

### 区块最小化 {#minimize-blocks}

区块可以最小化。当前 Tab 右下角会出现一个浮动的 minimized blocks 控制按钮：
...
### 区块复制到其他 Tab {#copy-a-block-to-another-tab}

区块可以在 Tab 之间复制：

- 把区块复制到指定 Tab。
- 把区块复制到新 Tab。
- 复制后的区块保留原始 meta 和视图信息。
...
很适合把某个项目或远程会话单独放到另一块屏幕。

### 回到 Tab 与布局修复 {#returning-to-a-tab-and-layout-fixes}

fork 还改善了若干布局细节：

- 从 Files 打开文件后，回到相关 Tab 更自然。
- 定向打开与区块移动菜单结合。
- 修复部分布局与焦点状态的异常情况。
- 终端 xterm 焦点恢复更稳定。
